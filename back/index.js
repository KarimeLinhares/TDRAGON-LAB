...
// Função de log no S3
async function logS3(logMessage) {
	const params = {
...
app.post('/login', async (req, res) => {
	...
	try {
		...
		await logS3(`Usuário ${username} logado.`);
		res.json({ token });
	} catch (error) {
		await logS3(`Falha de login do usuário ${username}.`);
...
sequelize
	.sync()
	.then(() => console.log('Sincronização com Banco de dados realizada'));
